...
"""
ProcessData.py with:
- Data aggregation from Equity / Delivery / F&O
- SD-based directional signals for Price / Delivery / OI (diagnostics)
- ML Option D (classifier + regressor) trained on every run
- XGBoost label remapping: [-1,0,1] <-> [0,1,2] for classification
- Separate RandomForest for weighted-median threshold extraction
- ALL ML thresholds for ALL features saved into configProcess.ini under [THRESHOLDS_<SYMBOL>]
- No SD fallback, no threshold mirroring: config contains only real ML thresholds
- Comma-cleaning of numeric columns
- DATE normalization across Equity/Delivery/F&O
"""
...
EQUITY_FILE_NAME = cfg.get("EQUITY_FILE_NAME")
DELIVERY_FILE_NAME = cfg.get("DELIVERY_FILE_NAME")
...
# ------------------------- MATRIX MAPPING (fallback explainability) ---
MATRIX_MAPPING = {
...
# ------------------ MODEL THRESHOLD EXTRACTION -------------------------
def extract_thresholds_from_random_forest(rf, feature_cols, target_class_index):
    """
    For RandomForestClassifier:
    Traverse each tree and collect thresholds for features for nodes whose
    subtree majority class is target_class_index. Return per-feature weighted medians.
    """
    from sklearn.tree import _tree

    feat_thresh = defaultdict(list)  # feature_name -> list of (threshold, weight)
    ...
    for est in rf.estimators_:
        ...
        def recurse(node):
            ...
            left_major = (
                ...
                else int(np.argmax(tree.value[left][0]))
            )
            right_major = (
                recurse(right)
                if right != _tree.TREE_LEAF
                else int(np.argmax(tree.value[right][0]))
            )
            node_value = tree.value[node][0]
            maj_class = int(np.argmax(node_value))
            if maj_class == target_class_index:
                feat = tree.feature[node]
                thr = tree.threshold[node]
                cover = float(node_value.sum())
                if feat >= 0 and feat < len(feature_cols) and not math.isnan(thr):
                    feat_name = feature_cols[feat]
                    feat_thresh[feat_name].append((thr, cover))
            return maj_class
    ...
    results = {}
    for feat_name, tv in feat_thresh.items():
        thresholds_vals = [t for t, w in tv]
        weights = [w for t, w in tv]
        wm = weighted_median(thresholds_vals, weights)
        if wm is not None and not math.isnan(wm):
            results[feat_name] = float(wm)
    return results


def extract_weighted_median_thresholds(clf, feature_cols):
    """
    Extract thresholds for class=1 and class=-1 from a RandomForestClassifier.
    Returns dict: {'long': {feat: thr}, 'short': {feat: thr}}
    """
    ...
    try:
        ...
        if isinstance(clf, RandomForestClassifier):
            print("Extracting thresholds from RandomForest (per-class weighted median).")
            classes = list(clf.classes_)
            label_to_index = {c: i for i, c in enumerate(classes)}
            # We expect original labels -1,0,1 here
            if -1 in label_to_index and 1 in label_to_index:
                long_idx = label_to_index[1]
                short_idx = label_to_index[-1]
                long_thr = extract_thresholds_from_random_forest(
                    clf, feature_cols, long_idx
                )
                short_thr = extract_thresholds_from_random_forest(
                    clf, feature_cols, short_idx
                )
                thresholds["long"] = long_thr
                thresholds["short"] = short_thr
            else:
                print(
                    "RF classes do not include -1 and 1; computing global per-feature medians."
                )
                ...
                for est in clf.estimators_:
                    ...
                    for node in range(tree.node_count):
                        ...
                        thr = tree.threshold[node]
                        if feat >= 0 and feat < len(feature_cols) and not math.isnan(thr):
                            cover = float(tree.value[node][0].sum())
                            feat_name = feature_cols[feat]
                            feat_map[feat_name].append((thr, cover))
                for feat_name, tv in feat_map.items():
                    thresholds_vals = [t for t, w in tv]
                    weights = [w for t, w in tv]
                    wm = weighted_median(thresholds_vals, weights)
                    if wm is not None and not math.isnan(wm):
                        thresholds["long"][feat_name] = float(wm)
                        thresholds["short"][feat_name] = float(wm)
    except Exception as e:
        print("Threshold extraction failed:", e)

    return thresholds


def normalize_feature_key(feat_name: str) -> str:
    """
    Normalize a feature name into a safe config key base:
    - remove leading '~'
    - non-alphanumeric -> '_'
    - lowercase
    """
...
def process_analysis_data(target_directory, sd_multiplier=SD_MULTIPLIER, trading_qty=TRADING_QTY):
    """
    Aggregates data, computes metrics, trains ML (every run),
    extracts weighted-median thresholds via RandomForest, saves them to config,
    and writes output CSV/Excel.
    """
    print("--- 1. Starting Data Processing and Calculation (Aggregating Files) ---")

    # --- 1.1 Load Equity Data ---
    ...
    for fn in equity_files:
        try:
            ...
            # Remove comma formatting in numeric-like columns
    ...
    print(
        f"Successfully loaded and merged {len(equity_df_list)} equity dataframes. "
        f"Total rows: {len(equity_df)}"
    )

    # --- 1.2 Load Delivery Data ---
    ...
    if not delivery_files:
        ...
        if delivery_df_list:
            ...
            print(
                f"Successfully loaded and merged {len(delivery_df_list)} delivery dataframes. "
                f"Total rows: {len(delivery_df)}"
            )
    ...
    if fao_files:
        ...
        for fn in fao_files:
            try:
                df_tmp = pd.read_csv(fn, encoding="utf-8-sig")
                # Dates like "31-Jul-25" (dd-MMM-yy) handled by _clean_dataframe
                df_tmp = _clean_dataframe(df_tmp, DATE_COL_CANDIDATES)
    ...
    # --- DATE normalization across all dataframes ---
    if "DATE" in equity_df.columns:
    ...
    final_df["Daily_Open_Interest_Sum"] = final_df.get(
        "Daily_Open_Interest_Sum", 0
    ).fillna(0)
    ...
    cleaned_prices = (
        final_df[EQUITY_CLOSE_PRICE_COL]
        .astype(str)
        .str.replace(",", "", regex=False)
        .str.replace(r"[^\d\.\-]", "", regex=True)
        .str.strip()
    )
    final_df[EQUITY_CLOSE_PRICE_COL] = pd.to_numeric(cleaned_prices, errors="coerce").fillna(0)
    ...
    delivery_qty_cleaned = (
        final_df[DELIVERY_QTY_FINAL_COL]
        .astype(str)
        .str.replace(",", "", regex=False)
        .str.replace(r"[^\d\.\-]", "", regex=True)
        .replace("", "0")
    ...
    # Directional signals (for diagnostics/features)
    ...
    # ------------------ ML Option D + Threshold Extraction ------------------
    try:
        ...
        print("Falling back to simple rule-based Longs/Shorts.")
        final_df["Longs"] = 0
        final_df["Shorts"] = 0
        ...
        final_df.loc[final_df["F&O_Conclusion"].isin(long_conditions), "Longs"] = final_df[
            "Absolute_OI_Change"
        ]
        final_df.loc[final_df["F&O_Conclusion"].isin(short_conditions), "Shorts"] = final_df[
            "Absolute_OI_Change"
        ]
        ...
        FUTURE_DAYS = int(cfg.get("ML_FUTURE_DAYS", 3))
        RETURN_THRESHOLD = float(cfg.get("ML_RETURN_THRESHOLD", 0.01))
        OI_QUANTILE = float(cfg.get("ML_OI_QUANTILE", 0.6))
        MIN_TRAIN_ROWS = int(cfg.get("ML_MIN_TRAIN_ROWS", 200))
        RANDOM_STATE = int(cfg.get("ML_RANDOM_STATE", 42))

        ml_df = final_df.copy()
        for c in [
            EQUITY_CLOSE_PRICE_COL,
            "Daily_Open_Interest_Sum",
            "Absolute_OI_Change",
            DELIVERY_VALUE_COL,
            NEW_5DAD_COL,
            REL_DELIVERY_COL,
        ...
        ml_df["future_return_pct"] = (
            (ml_df["future_close"] - ml_df[EQUITY_CLOSE_PRICE_COL])
            / ml_df[EQUITY_CLOSE_PRICE_COL]
        ...
        positive_future_oi = ml_df_labels.loc[
            ml_df_labels["future_oi_change"] > 0, "future_oi_change"
        ]
        if len(positive_future_oi) > 0:
            oi_thresh = positive_future_oi.quantile(OI_QUANTILE)
        else:
            oi_thresh = (
                ml_df_labels["future_oi_change"].median() if len(ml_df_labels) > 0 else 0.0
            )
        ...
        if len(ml_df_labels) == 0:
            print("ML: No rows for label construction. Using rule-based Longs/Shorts.")
            final_df["Longs"] = 0
            final_df["Shorts"] = 0
            long_conditions = ["NewLongs", "LongCovering", "StrongLong", "LastLegOfLong"]
            short_conditions = ["NewShort", "ShortCovering", "StrongShort", "LastLegOfshort"]
            ...
            if len(ml_df_labels) < MIN_TRAIN_ROWS or y.nunique() == 1:
                print(
                    "ML training skipped (not enough data or single-class labels). "
                    "Using rule-based Longs/Shorts."
                )
                final_df["Longs"] = 0
                final_df["Shorts"] = 0
                long_conditions = ["NewLongs", "LongCovering", "StrongLong", "LastLegOfLong"]
                ...
                # --- Main classifier (XGBoost or RF) with label remap for XGB ---
                if XGB_AVAILABLE:
                    clf = XGBClassifier(
                    ...
                    # XGBoost cannot use labels -1,0,1 directly -> remap to 0,1,2
                    y_for_clf = y.replace({-1: 0, 0: 1, 1: 2})
                else:
                ...
                # --- RandomForest for thresholds (trained on original labels -1,0,1) ---
                rf_for_thresholds = RandomForestClassifier(
                    n_estimators=200,
                    random_state=RANDOM_STATE,
                    n_jobs=-1,
                )
                rf_for_thresholds.fit(X_scaled, y)
                ...
                try:
                    ...
                    if XGB_AVAILABLE:
                        y_test_orig = pd.Series(y_train.index.map(  # careful but ok
                            lambda idx: y_for_clf.iloc[idx] if idx in y_for_clf.index else 1
                        ))
                        # simpler: just invert mapping on y_test & y_pred:
                        y_test_orig = pd.Series(y_test).replace({0: -1, 1: 0, 2: 1})
                        y_pred_orig = pd.Series(y_pred).replace({0: -1, 1: 0, 2: 1})
                    else:
                        y_test_orig = y_test
                        y_pred_orig = y_pred
                    print(
                        ...
                        classification_report(
                            y_test_orig, y_pred_orig, zero_division=0
                        ),
                    )
                except Exception:
                    pass
                ...
                if reg_features_mask.sum() >= 50:
                    X_reg = scaler.transform(
                        ml_df_labels.loc[reg_features_mask, feature_cols].fillna(0)
                    )
                    y_reg = ml_df_labels.loc[reg_features_mask, "future_oi_change"].abs()
                    if XGB_AVAILABLE:
                        reg = XGBRegressor(
                            random_state=RANDOM_STATE,
                            n_estimators=200,
                    ...
                    try:
                        print(
                            "Regressor MAE on training set:",
                            mean_absolute_error(y_reg, reg.predict(X_reg)),
                        )
                    except Exception:
                        pass
                else:
                ...
                raw_preds = clf.predict(all_X_scaled)

                if XGB_AVAILABLE:
                    ml_preds = pd.Series(raw_preds).replace({0: -1, 1: 0, 2: 1}).values
                else:
                    ml_preds = raw_preds
                ...
                if len(pred_mags) == len(final_df):
                    final_df.loc[final_df["ML_Label"] == 1, "Pred_Long_Value"] = pred_mags[
                        final_df["ML_Label"] == 1
                    ]
                    final_df.loc[final_df["ML_Label"] == -1, "Pred_Short_Value"] = pred_mags[
                        final_df["ML_Label"] == -1
                    ]
                else:
                ...
                # ---- Threshold extraction using RandomForest (ML-only) ----
                print("Extracting weighted-median thresholds from RandomForest...")
                extracted = extract_weighted_median_thresholds(rf_for_thresholds, feature_cols)
                long_feat = extracted.get("long", {}) or {}
                short_feat = extracted.get("short", {}) or {}

                # Build config thresholds dict: ALL ML thresholds, no fallback
                cfg_thresholds = {}

                # Classic three, if ML produced them
                if PRICE_CHANGE_COL in long_feat:
                    cfg_thresholds["price_long"] = long_feat[PRICE_CHANGE_COL]
                if REL_DELIVERY_COL in long_feat:
                    cfg_thresholds["del_long"] = long_feat[REL_DELIVERY_COL]
                if OI_CHANGE_COL in long_feat:
                    cfg_thresholds["oi_long"] = long_feat[OI_CHANGE_COL]

                if PRICE_CHANGE_COL in short_feat:
                    cfg_thresholds["price_short"] = short_feat[PRICE_CHANGE_COL]
                if REL_DELIVERY_COL in short_feat:
                    cfg_thresholds["del_short"] = short_feat[REL_DELIVERY_COL]
                if OI_CHANGE_COL in short_feat:
                    cfg_thresholds["oi_short"] = short_feat[OI_CHANGE_COL]

                # Full per-feature thresholds (Option B)
                for feat_name, thr in long_feat.items():
                    key_base = normalize_feature_key(feat_name)
                    key = f"{key_base}_long"
                    cfg_thresholds[key] = thr

                for feat_name, thr in short_feat.items():
                    key_base = normalize_feature_key(feat_name)
                    key = f"{key_base}_short"
                    cfg_thresholds[key] = thr

                # No SD fallback, no mirroring: ONLY ML thresholds get saved

                thresholds_section = f"THRESHOLDS_{SYMBOL}"
                config = configparser.ConfigParser()
                if os.path.exists(CONFIG_PATH):
                    try:
                        config.read(CONFIG_PATH)
                    except Exception as e:
                        print("Could not read config for threshold save:", e)

                try:
                    if not config.has_section(thresholds_section):
                        config.add_section(thresholds_section)
                    for key, val in cfg_thresholds.items():
                        if val is None:
                            continue
                        config.set(thresholds_section, key, str(float(val)))
                    with open(CONFIG_PATH, "w") as cfgfile:
                        config.write(cfgfile)
                    print(
                        f"Saved ML thresholds into config [{thresholds_section}] at {CONFIG_PATH}"
                    )
                except Exception as e:
                    print("Could not save thresholds to config:", e)

    # ------------------ END ML + Threshold Extraction -------------------
