# Added universal numeric cleaning
...
- Aggregates data from Equity / Delivery / F&O (same as ProcessData.py)
- Computes all derived columns (~Price, ~Del, ~OI, etc.)
- Trains ML (classifier + regressor) on every run
- Extracts weighted-median thresholds from a RandomForest
- Saves ALL ML thresholds for ALL features into configProcess.ini under [THRESHOLDS_<SYMBOL>]
- Does NOT generate *_Analysis.csv or *_Analysis_Excel.xlsx
"""
...
print("Using configuration (TrainModel.py):")
...
def build_base_dataframe(target_directory, sd_multiplier=SD_MULTIPLIER):
    """
    Aggregates Equity, Delivery, F&O and computes:
    - close, vwap, Delivery, 5DAD, ~Price, ~Del, ~OI
    - Directional signals & F&O_Conclusion, Trade_Signal
    Returns final_df (no ML, no thresholds, no file output).
    """
    print("--- TrainModel: Building base dataframe (no outputs) ---")
    ...
    # Directional signals (diagnostics only)
...
# ---------------------- TRAIN + SAVE THRESHOLDS ------------------------

def train_and_save_thresholds(target_directory, sd_multiplier=SD_MULTIPLIER):
    """
    Performs:
    - Build base dataframe
    - ML training (classifier + regressor)
    - Threshold extraction via RandomForest
    - Saves ALL thresholds to config [THRESHOLDS_<SYMBOL>]
    No *_Analysis files are written here.
    """
    final_df = build_base_dataframe(target_directory, sd_multiplier)

    # --- ML Option D + Threshold Extraction (unchanged logic) ---
    try:
        ...
        print("Cannot train ML models; thresholds will NOT be saved.")
        return final_df
    ...
    if len(ml_df_labels) == 0:
        print("ML: No rows for label construction. Cannot train; thresholds not saved.")
        return final_df
    ...
    if len(ml_df_labels) < MIN_TRAIN_ROWS or y.nunique() == 1:
        print(
            ...
            "Thresholds will not be updated."
        )
        return final_df
    ...
    # --- Classifier (XGB or RF) with label remap for XGB ---
    if XGB_AVAILABLE:
        clf = XGBClassifier(
    ...
    try:
        ...
        print(
            ...
            classification_report(y_test_orig, y_pred_orig, zero_division=0),
        )
    except Exception:
        pass

    # --- Regressor for magnitude (optional) ---
    reg = None
    ...
    try:
        ...
        print(f"Saved ML thresholds into config [{thresholds_section}] at {CONFIG_PATH}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    df = train_and_save_thresholds(TARGET_DIRECTORY, SD_MULTIPLIER)
    print("TrainModel.py finished. Thresholds updated in config (if training succeeded).")
